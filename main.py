...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        pathogens_data = [
            ...
            ("S. epidermidis", json.dumps(["Ванкомицин", "Линезолид", "Рифампицин", "Даптомицин"])),
            ("S. haemolyticus", json.dumps(["Ванкомицин", "Линезолид", "Даптомицин"])),
            ("S. lugdunensis", json.dumps(["Оксациллин", "Ванкомицин", "Цефтриаксон", "Клиндамицин"])),
            ("S. pyogenes", json.dumps(["Пенициллин G", "Цефтриаксон", "Клиндамицин", "Эритромицин"])),
            ("S. agalactiae", json.dumps(["Пенициллин G", "Цефтриаксон", "Ванкомицин", "Клиндамицин"])),
            ("S. dysgalactiae", json.dumps(["Пенициллин G", "Цефтриаксон", "Клиндамицин"])),
            ("S. anginosus", json.dumps(["Пенициллин G", "Цефтриаксон", "Клиндамицин", "Метронидазол"])),
            ("S. constellatus", json.dumps(["Пенициллин G", "Цефтриаксон", "Клиндамицин", "Метронидазол"])),
            ("S. intermedius", json.dumps(["Пенициллин G", "Цефтриаксон", "Клиндамицин", "Метронидазол"])),
            ...
            ("Corynebacterium jeikeium", json.dumps(["Ванкомицин", "Линезолид", "Даптомицин"])),
            ("Listeria monocytogenes", json.dumps(["Ампициллин", "Гентамицин", "Триметоприм/сульфаметоксазол"])),
            ("Clostridium difficile", json.dumps(["Метронидазол", "Ванкомицин (перорально)", "Фидаксомицин"])),
        ]
